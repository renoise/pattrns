...
/// C lang compatible Result<Pattern, String> representation for
/// new_pattern_from_string/file. Error Strings must be deleted with
/// `drop_error_string`. Pattern values must be deleted with `drop_pattern`,
struct PatternResult {
  ...
  bool IsValue() const { return tag == Tag::Value; }
...
/// C lang compatible representation of a rust
/// `Vec<pattrns::ParameterChangeEvent>`.
...
/// Create a new pattern from the given script file path, using the given beat
/// time and instrument. The returned pattern result must be deleted via
/// `drop_pattern` or `drop_error_string`.
PatternResult new_pattern_from_file(Timebase time_base,
                                    const uint32_t *instrument_id,
                                    const char *file_name);

/// Create a new pattern from the given script contents, using the given beat
/// time and instrument. The returned pattern result must be deleted via
/// `drop_pattern` or `drop_error_string`.
...
/// Create a new resetted clone from an existing pattern with the given timebase
/// and instrument id. The returned pattern result must be deleted via
/// `drop_pattern` or `drop_error_string`.
PatternResult new_pattern_instance(Pattern *this_, Timebase time_base);

/// Get parameters of a pattern.
/// The returned result must be deleted via `drop_parameter_set` or
/// `drop_error_string`.
ParameterSetResult pattern_parameters(Pattern *this_);

/// Set a single parameter value of a pattern.
VoidResult set_pattern_parameter_value(Pattern *this_, const char *id,
                                       double value);
...
VoidResult run_pattern(Pattern *this_, void *callback_context,
                       void (*callback)(void *, const PatternPlaybackEvent *));

/// Run pattern, consuming all events which the pattern generated up to given
/// sample time. NB: Events are only valid within the callback, so they must be
/// consumed or copied when used outside of the callback.
VoidResult
run_pattern_until_time(Pattern *this_, uint64_t time, void *callback_context,
                       void (*callback)(void *, const PatternPlaybackEvent *));
...
/// Initialize lib and set external allocator, which should be used instead of
/// the system allocator as global allocator (unless the "dhat-profiler" feature
/// is enabled).
